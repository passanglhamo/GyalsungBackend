        ...
        if (deferEx.equals('E')) {
            ...
            if (!Objects.isNull(exemptionInfo)) {
                if (exemptionInfo.size() != 0) {
                    ...
                    responseMessage.setMessage("There is already one pending exemption application. In order to add a new application, please contact Gyalsung head office.");
            ...
            if (!Objects.isNull(exemptionInfoApprove)) {
                if (exemptionInfo.size() != 0) {
                    responseMessage.setStatus(ApprovalStatus.APPROVED.value());
                    responseMessage.setMessage("There is already one approved exemption application. In order to add a new application, please contact Gyalsung head office.");
                    return new ResponseEntity<>(responseMessage, HttpStatus.ALREADY_REPORTED);
                }
        ...
        if (deferEx.equals('D')) {
            ...
            if (!Objects.isNull(defermentInfo)) {
                if (defermentInfo.size() != 0) {
                    responseMessage.setStatus(ApprovalStatus.APPROVED.value());
                    responseMessage.setMessage("There is already one pending deferred application. In order to add a new application, please contact Gyalsung head office.");
                    return new ResponseEntity<>(responseMessage, HttpStatus.ALREADY_REPORTED);
                }
            }
            ...
            if (!Objects.isNull(defermentInfoApprovalInYear)) {
                responseMessage.setStatus(ApprovalStatus.APPROVED.value());
                responseMessage.setMessage("There is already one approved deferred application for selected year. In order to add a new application, please contact Gyalsung head office.");
                return new ResponseEntity<>(responseMessage, HttpStatus.ALREADY_REPORTED);
            }
        }
