...
public class CaseNumberGenerator {
    ...
    public String caseNumberGenerator(String authHeader, BigInteger reasonId) {
        ...
        DefermentInfo defermentInfo = repository.findByReasonIdOrderByIdDesc(reasonId);
        if(Objects.isNull(defermentInfo)){
            return formattedDate+"/D"+Objects.requireNonNull(responseTraining.getBody()).getCode()+"/"+initialRandomNo;
        }
        String caseNumber = defermentInfo.getCaseNumber();
        ...
        return formattedDate+"/D"+Objects.requireNonNull(responseTraining.getBody()).getCode()+"/"+value;
    }
}
