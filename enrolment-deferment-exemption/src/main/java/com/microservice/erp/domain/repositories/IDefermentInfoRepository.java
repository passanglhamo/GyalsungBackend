...
public interface IDefermentInfoRepository extends JpaRepository<DefermentInfo, BigInteger> {
 @Query(value = "SELECT d.* FROM ede_deferment_info d " +
         ...
         "AND (:gender IS NULL OR CAST(d.gender AS CHAR(1))=CAST(:gender AS CHAR(1))) AND (:caseNumber IS NULL OR CAST(d.caseNumber AS VARCHAR (20))=CAST(:caseNumber AS VARCHAR(20))) AND (:reasonId IS NULL OR CAST(d.reason_id AS CHAR(255)) =CAST(:reasonId AS CHAR(255)))", nativeQuery = true)
 ...
 DefermentInfo getDefermentByUserIdNotRejected(BigInteger userId,Character rejectStatus);
    ...
    DefermentInfo findByReasonIdAndApplicationDateOrderByIdDesc(BigInteger reasonId,Date applicationDate);
